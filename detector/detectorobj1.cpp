...
{
    roi.x = obj::ROI_X;
    roi.y = VIDEO_FRAME_HEIGHT/ 5;//obj::ROI_DIV_Y;
...
//          closest = color;
		  if (color[0] ==60 || color[0]==120)//select only red
		  {
			 closest = color;
		  }
		  else
...
//    cv::blur(src, src, cv::Size(3, 3));
    ...
    cv::rectangle(binary, cv::Point(0, h/ 2.5 /*obj::ROI_FLOOR_DIV*/), cv::Point(w, h), cv::Scalar(0), -1);
...
//     cv::rectangle(src, cv::Point(0, h/ 2.5 /*obj::ROI_FLOOR_DIV*/), cv::Point(w, h), cv::Scalar(0), -1);
